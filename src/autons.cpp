// Intent - Autonomous functions

#include "autons.hpp"
#include "EZ-Template/util.hpp"
#include "constants.hpp"
#include "main.h" // IWYU pragma: keep
#include "subsystems.hpp"

/////
// For installation, upgrading, documentations, and tutorials, check out our website!
// https://ez-robotics.github.io/EZ-Template/
/////

void testing_pid_auto()
{
  using namespace StratusQuo;
  chassis.drive_angle_set(0_deg);
  chassis.pid_drive_set(48_in, 110);
  chassis.pid_wait();
}

#pragma region Goal Rushes
void blue_side_goal_rush()
{
  using namespace StratusQuo;
  chassis.pid_drive_set(40_in, 127);
  intake.move(127);
  chassis.pid_wait_until(10_in);
  intake.toggle();
  chassis.pid_wait_until(24_in);
  intake.toggle();
  chassis.pid_wait_quick_chain();
  right_doinker.set(true);
  pros::delay(200);
  intake.get_hook_motor().brake();
  pros::delay(550);
  chassis.pid_drive_set(-12_in, 127);
  chassis.pid_wait_quick();
  right_doinker.set(false);
  pros::delay(450);
  chassis.pid_turn_set(180_deg, 127);
  intake.get_roller_motor().brake();
  chassis.pid_wait_quick();
  chassis.pid_drive_set(-16_in, 127);
  chassis.pid_wait();
  set_clamp.store(true);
  intake.get_roller_motor().move(127);
  intake.get_hook_motor().move(-127);
  chassis.pid_turn_set(90_deg, 110);
  intake.move(127);
  chassis.pid_wait();
  set_clamp.store(false);
  chassis.pid_turn_set(300_deg, 127);
  chassis.pid_wait();
  chassis.pid_drive_set(-28_in, 127);
  chassis.pid_wait();
  set_clamp.store(true);
  intake.get_roller_motor().move(127);
  intake.get_hook_motor().move(-127);
  chassis.pid_turn_set(133_deg, 110);
  chassis.pid_wait_until(90_deg);
  intake.move(127);
  chassis.pid_wait_quick_chain();
  intake.toggle();
  chassis.pid_drive_set(35_in, 127);
  intake.move(127);
  chassis.pid_wait();
  intake.toggle();
  chassis.pid_drive_set(-12_in, 40);
  chassis.pid_wait();
  chassis.pid_turn_set(250_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(54_in, 127);
  chassis.pid_wait();
}


void red_side_goal_rush()
{
  // TODO: Lower speed, then try to optimize with speed.
  using namespace StratusQuo;
  chassis.pid_drive_set(40_in, 127);
  intake.move(127);
  chassis.pid_wait_until(32_in);
  intake.get_hook_motor().brake();
  chassis.pid_wait_quick_chain();
  right_doinker.set(true);
  pros::delay(750);
  chassis.pid_drive_set(-12_in, 127);
  chassis.pid_wait_quick_chain();
  right_doinker.set(false);
  pros::delay(450);
  chassis.pid_turn_set(180_deg, 127);
  intake.brake();
  chassis.pid_wait_quick();
  chassis.pid_drive_set(-16_in, 127);
  chassis.pid_wait();
  set_clamp.store(true);
  intake.get_hook_motor().move(-127);
  chassis.pid_turn_set(270_deg, 110);
  intake.move(127);
  chassis.pid_wait();
  set_clamp.store(false);
  chassis.pid_turn_set(60_deg, 127);
  chassis.pid_wait();
  chassis.pid_drive_set(-24_in, 127);
  chassis.pid_wait();
  set_clamp.store(true);
  intake.get_hook_motor().move(-127);
  chassis.pid_turn_set(227_deg, 110);
  chassis.pid_wait();
  intake.toggle();
  chassis.pid_drive_set(34_in, 127);
  intake.move(127);
  chassis.pid_wait();
  intake.toggle();
  chassis.pid_drive_set(-12_in, 40);
  chassis.pid_wait();
  chassis.pid_turn_set(110_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(50_in, 127);
  chassis.pid_wait();
}

void red_side_fast_goal_rush()
{
  using namespace StratusQuo;
  chassis.drive_angle_set(45_deg);
  chassis.pid_drive_set(38_in, 127);
  chassis.pid_wait();
  left_doinker.set(true);
  pros::delay(500);
  chassis.pid_turn_set(100_deg, 110, true);
  chassis.pid_wait();
  left_doinker.set(false);
  chassis.pid_drive_set(-12_in, 127);
  chassis.pid_wait();
  set_clamp.store(true);
  pros::delay(100);
  chassis.pid_wait();
  intake.move(127);
  pros::delay(800);
  chassis.pid_wait();
  set_clamp.store(false);
  chassis.pid_wait();
  chassis.pid_turn_set(140_deg, 110, true);
  chassis.pid_wait();
  chassis.pid_drive_set(31_in, 127);
  chassis.pid_wait();
  chassis.pid_drive_set(-4_in, 127);
  chassis.pid_wait();
  chassis.pid_turn_set(220_deg, 110, true);
  chassis.pid_wait();
  chassis.pid_drive_set(-12_in, 127);
  chassis.pid_wait();
  set_clamp.store(true);
  intake.get_hook_motor().move(-127);
  chassis.pid_wait();
  chassis.pid_turn_set(272_deg, 110, true);
  chassis.pid_wait();
  chassis.pid_drive_set(40_in, 127);
  chassis.pid_wait_quick_chain();
  intake.toggle();
  intake.move(127);
  chassis.pid_wait();
  chassis.pid_drive_set(18_in, 50); 
  chassis.pid_wait();
  intake.toggle();
  chassis.pid_wait();
  chassis.pid_turn_set(390_deg, 60);
  chassis.pid_wait();
  chassis.pid_drive_set(32_in, 110, true);
}
#pragma endregion

#pragma region Ring Rushes
void blue_side_ring_rush()
{
  using namespace StratusQuo;
  chassis.drive_angle_set(0_deg);
  right_doinker.set(true);
  chassis.pid_drive_set(47_in, 127);
  chassis.pid_wait_quick_chain();
  chassis.pid_drive_set(-24_in, 110);
  chassis.pid_wait_quick();
  right_doinker.set(false);
  pros::delay(250);
  chassis.pid_turn_set(90_deg, 110);
  chassis.pid_wait_quick();
  chassis.pid_drive_set(-22_in, 110);
  chassis.pid_wait_quick();
  intake.move(-127);
  set_clamp.store(true);
  chassis.pid_turn_set(52.5_deg, 110);
  chassis.pid_wait_quick();
  intake.brake();
  chassis.pid_drive_set(44_in, 60);
  intake.move(127);
  chassis.pid_wait();
  chassis.pid_drive_set(-24_in, 110);
  chassis.pid_wait();
  intake.brake();
  chassis.pid_turn_set(190_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(44_in, 110);
  intake.move(127);
  chassis.pid_wait();
  chassis.pid_turn_set(275_deg, 110);
  chassis.pid_wait();
  intake.toggle();
  chassis.pid_drive_set(11_in, 90);
  chassis.pid_wait();
  intake.toggle();
  pros::delay(400);
  chassis.pid_drive_set(-11_in, 40);
  chassis.pid_wait();
  chassis.pid_drive_set(8.5_in, 90);
  chassis.pid_wait();
  chassis.pid_turn_set(340_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(28_in, 110);
}


void red_side_ring_rush()
{
  using namespace StratusQuo;
  right_doinker.set(true);
  chassis.pid_drive_set(50_in, 127);
  chassis.pid_wait();
  chassis.pid_drive_set(-16_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(20_deg, 110);
  chassis.pid_wait();
  right_doinker.set(false);
  pros::delay(100);
  chassis.pid_drive_set(-8_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(-95_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(-18_in, 110);
  chassis.pid_wait();
  pros::delay(25);  
  set_clamp.store(true);
  intake.move(-127);
  chassis.pid_turn_set(-20_deg, 110);
  chassis.pid_wait();
  intake.move(127);
  chassis.pid_drive_set(12_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(-70_deg, 110);
  chassis.pid_wait_quick_chain();
  chassis.pid_drive_set(28_in, 40);
  chassis.pid_wait();
  chassis.pid_drive_set(-8_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(-205_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(48_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(-300_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(26_in, 110);
}

void red_side_psu_ring()
{
  using namespace StratusQuo;
  team_color = RED;
  lady_brown.set_pto(true);
  pros::delay(100);
  lady_brown.move(-127);
  pros::delay(325);
  lady_brown.move(127);
  pros::delay(325);
  lady_brown.set_pto(false);

  chassis.pid_drive_set(-24_in, 110);
  chassis.pid_wait_quick_chain();
  chassis.pid_turn_set(-200_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(24_in, 110);
  intake.get_roller_motor().move(127);
  chassis.pid_wait();
  chassis.pid_turn_set(-240_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(-24_in, 110);
  chassis.pid_wait();
  set_clamp.store(true);
  intake.get_hook_motor().move(-127);
  pros::delay(20);
  intake.move(127);
  chassis.pid_turn_set(-180_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(22_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(-240_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(26_in, 40);
  chassis.pid_wait();
}

void red_side_four_ring()
{
  using namespace StratusQuo;
  lady_brown.set_pto(true);
  pros::delay(100);
  lady_brown.move(-127);
  pros::delay(325);
  lady_brown.move(127);
  pros::delay(325);
  lady_brown.set_pto(false);

  chassis.pid_turn_set(-100_deg, 110);
  intake.toggle();
  intake.get_roller_motor().move(127);
  chassis.pid_wait();
  chassis.pid_drive_set(15_in, 50);
  chassis.pid_wait();
  intake.toggle();
  pros::delay(200);
  chassis.pid_drive_set(-12_in, 40);
  chassis.pid_wait();
  chassis.pid_turn_set(15_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(-32_in, 110);
  chassis.pid_wait();
  set_clamp.store(true);
  intake.move(127);
  chassis.pid_turn_set(180_deg, 50);
  chassis.pid_wait();
  chassis.pid_drive_set(25_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(120_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(26_in, 50);
  chassis.pid_wait();
  chassis.pid_drive_set(-6_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(0_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(24_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(110_deg, 110);
  intake.move(127);
  chassis.pid_wait();
  chassis.pid_drive_set(58_in, 110);
}

void blue_side_four_ring()
{
  using namespace StratusQuo;
  lady_brown.set_pto(true);
  pros::delay(100);
  lady_brown.move(-127);
  pros::delay(325);
  lady_brown.move(127);
  pros::delay(325);
  lady_brown.set_pto(false);

  chassis.pid_turn_set(100_deg, 110);
  intake.toggle();
  intake.get_roller_motor().move(127);
  chassis.pid_wait();
  chassis.pid_drive_set(15_in, 50);
  chassis.pid_wait();
  intake.toggle();
  pros::delay(200);
  chassis.pid_drive_set(-12_in, 40);
  chassis.pid_wait();
  chassis.pid_turn_set(-15_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(-32_in, 110);
  chassis.pid_wait();
  set_clamp.store(true);
  intake.move(127);
  chassis.pid_turn_set(180_deg, 50);
  chassis.pid_wait();
  chassis.pid_drive_set(25.5_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(230_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(25_in, 45);
  chassis.pid_wait();
  chassis.pid_drive_set(-6_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(355_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(16_in, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(-6_in, 110);
}
#pragma endregion

#pragma region AWPs
void solo_awp_left()
{
  using namespace StratusQuo;
  lady_brown.set_pto(true);
  pros::delay(100);
  lady_brown.move(-127);
  pros::delay(325);
  lady_brown.move(127);
  pros::delay(325);
  lady_brown.set_pto(false);

  chassis.pid_drive_set(-24_in, 127);
  chassis.pid_wait_quick_chain();
  chassis.pid_turn_set(45_deg, 127);
  chassis.pid_wait();
  chassis.pid_drive_set(-22_in, 110);
  chassis.pid_wait_quick_chain();
  intake.get_hook_motor().move(-127);
  intake.get_roller_motor().move(127);
  pros::delay(100);
  set_clamp.store(true);
  chassis.pid_turn_set(110_deg, 127);
  chassis.pid_wait_until(65_deg);
  intake.brake();
  chassis.pid_wait();
  intake.move(127);
  chassis.pid_drive_set(24_in, 127);
  chassis.pid_wait();
  chassis.pid_turn_set(-25.5_deg, 127);
  chassis.pid_wait();
  chassis.pid_drive_set(48.8_in, 110);
  intake.toggle();
  chassis.pid_wait_quick();
  intake.toggle();
  intake.brake();
  intake.get_roller_motor().move(127);
  set_clamp.store(false);
  chassis.pid_turn_set(87_deg, 50);
  chassis.pid_wait();
  chassis.pid_drive_set(-40_in, 127);
  chassis.pid_wait();
  intake.move(-127);
  intake.get_roller_motor().move(127);
  set_clamp.store(true);
  chassis.pid_turn_set(-50_deg, 127);
  chassis.pid_wait_until(0_deg);
  intake.move(127);
  chassis.pid_wait();
  chassis.pid_drive_set(17_in, 127);
  chassis.pid_wait_quick_chain();
  chassis.pid_turn_set(-40_deg, 127);
  chassis.pid_wait_quick();
  intake.brake();
  chassis.pid_drive_set(-40_in, 110);
  pros::delay(300);
  lady_brown.toggle();
}

void solo_awp_right()
{
  using namespace StratusQuo;
  lady_brown.set_pto(true);
  pros::delay(100);
  lady_brown.move(-127);
  pros::delay(325);
  lady_brown.move(127);
  pros::delay(325);
  lady_brown.set_pto(false);

  chassis.pid_drive_set(-24_in, 127);
  chassis.pid_wait_quick_chain();
  chassis.pid_turn_set(-45_deg, 127);
  chassis.pid_wait();
  chassis.pid_drive_set(-22_in, 110);
  chassis.pid_wait();
  intake.get_roller_motor().move(127);
  intake.get_hook_motor().move(-127);
  pros::delay(100);
  set_clamp.store(true);
  chassis.pid_turn_set(-110_deg, 127);
  chassis.pid_wait_until(-65_deg);
  intake.move(127);
  chassis.pid_drive_set(24_in, 127);
  chassis.pid_wait();
  chassis.pid_turn_set(26.5_deg, 127);
  chassis.pid_wait();
  chassis.pid_drive_set(48.9_in, 110);
  intake.toggle();
  intake.get_hook_motor().brake();
  chassis.pid_wait_quick();
  intake.toggle();
  set_clamp.store(false);
  chassis.pid_turn_set(-87.5_deg, 60);
  chassis.pid_wait();
  chassis.pid_drive_set(-40_in, 127);
  chassis.pid_wait();
  intake.get_hook_motor().move(-127);
  intake.get_roller_motor().move(127);
  set_clamp.store(true);
  chassis.pid_turn_set(50_deg, 127);
  chassis.pid_wait_until(0_deg);
  intake.move(127);
  chassis.pid_wait();
  chassis.pid_drive_set(17_in, 127);
  chassis.pid_wait_quick_chain();
  chassis.pid_turn_set(40_deg, 127);
  chassis.pid_wait_quick();
  intake.brake();
  chassis.pid_drive_set(-40_in, 127);
  pros::delay(300);
  lady_brown.toggle();
}

void half_awp()
{
  using namespace StratusQuo;
  lady_brown.set_pto(true);
  pros::delay(100);
  lady_brown.move(-127);
  pros::delay(325);
  lady_brown.move(127);
  pros::delay(325);
  lady_brown.set_pto(false);

  chassis.pid_drive_set(-24_in, 127);
  chassis.pid_wait_quick_chain();
  chassis.pid_turn_set(45_deg, 80);
  chassis.pid_wait();
  chassis.pid_drive_set(-22_in, 127);
  chassis.pid_wait_quick_chain();
  intake.get_hook_motor().move(-127);
  intake.get_roller_motor().move(127);
  pros::delay(100);
  set_clamp.store(true);
  chassis.pid_turn_set(108_deg, 80);
  chassis.pid_wait();
  intake.move(127);
  chassis.pid_drive_set(24_in, 127);
  chassis.pid_wait();
  chassis.pid_turn_set(-28.5_deg, 80);
  chassis.pid_wait();
  chassis.pid_drive_set(48.75_in, 127);
  intake.toggle();
  chassis.pid_wait_quick();
  intake.toggle();
  chassis.pid_turn_set(-135_deg, 40);
  chassis.pid_wait();
  chassis.pid_drive_set(24_in, 110);
}

void awp_no_ring()
{
  using namespace StratusQuo;
  lady_brown.set_pto(true);
  pros::delay(100);
  lady_brown.move(-127);
  pros::delay(325);
  lady_brown.move(127);
  pros::delay(325);
  lady_brown.set_pto(false);

  chassis.pid_drive_set(-24_in, 127);
  chassis.pid_wait_quick_chain();
  chassis.pid_turn_set(45_deg, 127);
  chassis.pid_wait();
  chassis.pid_drive_set(-22_in, 110);
  chassis.pid_wait_quick_chain();
  intake.get_roller_motor().move(127);
  intake.get_hook_motor().move(-127);
  pros::delay(100);
  set_clamp.store(true);
  chassis.pid_turn_set(110_deg, 127);
  chassis.pid_wait_until(65_deg);
  intake.brake();
  chassis.pid_wait();
  intake.move(127);
  chassis.pid_drive_set(24_in, 127);
  chassis.pid_wait();
  chassis.pid_turn_set(290_deg, 40);
  chassis.pid_wait();
  chassis.pid_drive_set(24_in, 110);
  lady_brown.toggle();
}

void right_side_half_awp()
{
  using namespace StratusQuo;
  pros::delay(3000);
  lady_brown.set_pto(true);
  pros::delay(100);
  lady_brown.move(-127);
  pros::delay(325);
  lady_brown.move(127);
  pros::delay(325);
  lady_brown.set_pto(false);
  chassis.pid_drive_set(-24_in, 127);
  chassis.pid_wait_quick_chain();
  chassis.pid_turn_set(-45_deg, 127);
  chassis.pid_wait();
  chassis.pid_drive_set(-22_in, 110);
  chassis.pid_wait();
  intake.get_roller_motor().move(127);
  intake.get_hook_motor().move(-127);
  pros::delay(100);
  set_clamp.store(true);
  chassis.pid_turn_set(-110_deg, 127);
  chassis.pid_wait_until(-65_deg);
  intake.move(127);
  chassis.pid_drive_set(24_in, 127);
  chassis.pid_wait();
  chassis.pid_turn_set(27_deg, 127);
  chassis.pid_wait();
  chassis.pid_drive_set(48.6_in, 110);
  intake.toggle();
  chassis.pid_wait();
  intake.toggle();
  pros::delay(200);
  chassis.pid_turn_set(145_deg, 40);
  chassis.pid_wait();
  chassis.pid_drive_set(24_in, 110);
}
#pragma endregion

#pragma region Skills
void first_autonomous_skills()
{
 using namespace StratusQuo;
chassis.drive_angle_set(180_deg);
chassis.pid_drive_set(-24_in, 60, true);
chassis.pid_wait();
set_clamp.store(true);
pros::delay(100); 
intake.move(127);
chassis.pid_drive_set(24_in, 60, true);
chassis.pid_wait();
chassis.pid_turn_set(270_deg, 110, true);
chassis.pid_wait();
chassis.pid_drive_set(24_in, 60, true);
chassis.pid_wait();
chassis.pid_turn_set(360_deg, 110, true);
chassis.pid_wait();
chassis.pid_drive_set(48_in, 60, true);
}

void second_autonomous_skills()
{
  using namespace StratusQuo;
  chassis.drive_angle_set(0_deg);
  intake.move(127);
  pros::delay(500);
  intake.brake();
  chassis.pid_drive_set(12_in, 60, true);
  chassis.pid_wait();
  chassis.pid_turn_set(90_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(-24_in, 110);
  chassis.pid_wait();
  set_clamp.store(true);
  intake.move(-127);
  chassis.pid_turn_set(-85_deg, 110);
  chassis.pid_wait();
  intake.move(127);
  chassis.pid_drive_set(20_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(-180_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(12_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(-45_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(24_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(45_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(-24_in, 110);
  intake.brake();
  chassis.pid_wait();
  set_clamp.store(false);
  intake.get_roller_motor().move(127);
  chassis.pid_drive_set(12_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(-88_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(-80_in, 110);
  chassis.pid_wait();
  set_clamp.store(true);
  intake.move(-127);
  chassis.pid_turn_set(90_deg, 110);
  chassis.pid_wait();
  intake.move(127);
  chassis.pid_drive_set(24_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(180_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(24_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(45_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(24_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(-15_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(-24_in, 110);
  chassis.pid_wait();
  intake.brake();
  set_clamp.store(false);
  intake.move(-127);
  chassis.pid_drive_set(116_in, 110);
  pros::delay(500);
  intake.get_roller_motor().move(127);
  chassis.pid_wait();
  chassis.pid_turn_set(90_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(-24_in, 110);
  chassis.pid_wait();
  set_clamp.store(true);
  pros::delay(100);
  intake.move(127);
  chassis.pid_wait();
  chassis.pid_drive_set(69_in, 110); // TODO: Set up auton
  chassis.pid_wait();
  chassis.pid_drive_set(-20_in, 110); // TODO: Set up auton
  chassis.pid_wait();
  chassis.pid_turn_set(240_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(-36_in, 60);
  chassis.pid_wait();
  set_clamp.store(false);
  intake.move(-127);
  chassis.pid_drive_set(24_in, 60);
  chassis.pid_wait();
  chassis.pid_turn_set(90_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(-60_in, 110);  
}

void six_ring_red()
{
  using namespace StratusQuo;
  team_color = RED;
  lady_brown.set_pto(true);
  pros::delay(100);
  lady_brown.move(-127);
  pros::delay(325);
  lady_brown.move(127);
  pros::delay(325);
  lady_brown.set_pto(false);
  chassis.pid_turn_set(-90,110);
  intake.toggle(); 
  intake.get_roller_motor().move(127);
  chassis.pid_wait();
  chassis.pid_drive_set(14_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(0_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(-33_in, 110);
  intake.move(127);
  chassis.pid_wait();
  set_clamp.store(true);
  chassis.pid_turn_set(40_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(18_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(-50_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(24_in, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(-12_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(-130_deg, 110); 
  chassis.pid_wait();
  chassis.pid_drive_set(24_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(-80_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(33_in, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(-33_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(135_deg, 110);
  chassis.pid_wait();
  chassis.pid_drive_set(72_in, 110);
  chassis.pid_wait();
}
#pragma endregion


void red_negative_four_ring()
{
  using namespace StratusQuo;
  lady_brown.set_pto(true);
  pros::delay(100);
  lady_brown.move(-127);
  pros::delay(325);
  lady_brown.move(127);
  pros::delay(325);
  lady_brown.set_pto(false);

  chassis.pid_drive_set(-6_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(-90_deg, 80);
  intake.get_roller_motor().move(127);
  chassis.pid_wait();
  intake.toggle();
  chassis.pid_drive_set(16_in, 80, true);
  chassis.pid_wait_quick();
  intake.toggle();
  pros::delay(200);
  chassis.pid_turn_set(-2.5_deg, 30);
  chassis.pid_wait();
  chassis.pid_drive_set(-44_in, 110, true);
  chassis.pid_wait();
  set_clamp.store(true);
  intake.get_hook_motor().move(-127);
  chassis.pid_turn_set(-180_deg, 80);
  chassis.pid_wait_until(-160_deg);
  intake.move(127);
  chassis.pid_wait();
  chassis.pid_swing_set(ez::RIGHT_SWING, -235_deg, 110, 25);
  chassis.pid_wait();
  chassis.pid_drive_set(22_in, 110);
  chassis.pid_wait_quick();
  chassis.pid_drive_set(-12_in, 110);
  chassis.pid_wait_quick();
  chassis.pid_swing_set(ez::RIGHT_SWING, -420_deg, 70, -15);
  chassis.pid_wait();
  chassis.pid_drive_set(48_in, 127);
  chassis.pid_wait();
  lady_brown.toggle();
}

void red_positive_four_ring()
{
  using namespace StratusQuo;
  lady_brown.set_pto(true);
  pros::delay(100);
  lady_brown.move(-127);
  pros::delay(325);
  lady_brown.move(127);
  pros::delay(325);
  lady_brown.set_pto(false);

  chassis.pid_drive_set(-24_in, 110);
  chassis.pid_wait_quick();
  chassis.pid_turn_set(-42.5_deg, 80);
  chassis.pid_wait();
  chassis.pid_drive_set(-24_in, 110);
  chassis.pid_wait_quick();
  intake.get_roller_motor().move(127);
  intake.get_hook_motor().move(-127);
  pros::delay(100);
  set_clamp.store(true);
  chassis.pid_turn_set(-110_deg, 80);
  chassis.pid_wait_until(-65_deg);
  intake.brake();
  chassis.pid_wait();
  intake.move(127);
  chassis.pid_drive_set(36_in, 110);
  chassis.pid_wait();
}
void blue_negative_four_ring()
{
  using namespace StratusQuo;
  lady_brown.set_pto(true);
  pros::delay(100);
  lady_brown.move(-127);
  pros::delay(325);
  lady_brown.move(127);
  pros::delay(325);
  lady_brown.set_pto(false);

  chassis.pid_drive_set(-6_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(90_deg, 80);
  intake.get_roller_motor().move(127);
  chassis.pid_wait();
  intake.toggle();
  chassis.pid_drive_set(16_in, 80, true);
  chassis.pid_wait_quick();
  intake.toggle();
  pros::delay(200);
  chassis.pid_turn_set(5_deg, 30);
  chassis.pid_wait();
  chassis.pid_drive_set(-44_in, 110, true);
  chassis.pid_wait();
  set_clamp.store(true);
  intake.get_hook_motor().move(-127);
  chassis.pid_turn_set(180_deg, 80);
  chassis.pid_wait_until(160_deg);
  intake.move(127);
  chassis.pid_wait();
  chassis.pid_swing_set(ez::LEFT_SWING, 235_deg, 110, 25);
  chassis.pid_wait();
  chassis.pid_drive_set(26_in, 110); // TODO: Set up auton
  chassis.pid_wait_quick();
  chassis.pid_drive_set(-12_in, 110);
  chassis.pid_wait_quick();
  chassis.pid_swing_set(ez::LEFT_SWING, 420_deg, 70, -15);
  chassis.pid_wait();
  chassis.pid_drive_set(48_in, 127);
  chassis.pid_wait_until(24_in);
  intake.get_hook_motor().brake();
}
void blue_positive_four_ring()
{
  using namespace StratusQuo;
  lady_brown.set_pto(true);
  pros::delay(100);
  lady_brown.move(-127);
  pros::delay(325);
  lady_brown.move(127);
  pros::delay(325);
  lady_brown.set_pto(false);

  chassis.pid_drive_set(-6_in, 110);
  chassis.pid_wait();
  chassis.pid_turn_set(-90_deg, 110);
  intake.get_roller_motor().move(127);
  chassis.pid_wait();
  intake.toggle();
  chassis.pid_drive_set(15_in, 110);
  chassis.pid_wait();
  intake.toggle();
  pros::delay(200);
  chassis.pid_turn_set(-2.5_deg, 30);
  chassis.pid_wait();
  chassis.pid_drive_set(-44_in, 110, true);
  chassis.pid_wait();
  set_clamp.store(true);
  intake.get_hook_motor().move(-127);
  chassis.pid_turn_set(-90_deg, 110);
  chassis.pid_wait();
}